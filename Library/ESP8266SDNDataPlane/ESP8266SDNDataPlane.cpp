/*
 * SDN Data Plane Library Implementation for ESP8266
 * Control Plane-Centric Architecture
 * Fixed for ESP8266 compatibility
 */
...
void SDNDataPlane::startAPMode() {
    // Generate unique AP name using ESP8266 MAC address
    uint8_t mac[6];
    WiFi.macAddress(mac);
    char macStr[13];
    sprintf(macStr, "%02X%02X%02X%02X%02X%02X", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
    
    String apName = "ESP8266_Device_" + String(macStr).substring(6); // Use last 6 chars of MAC
    String apPassword = loadAPPassword(); // Load saved password or default
...
void SDNDataPlane::handleOperational() {
    ...
    if (WiFi.status() != WL_CONNECTED) {
        ...
        if (WiFi.status() != WL_CONNECTED) {
            Serial.println("Failed to reconnect, reverting to AP mode");
            currentState = ERROR_STATE;
            return;
        } else {
            Serial.println("Reconnected successfully");
            registerWithControlPlane(); // Re-register
        }
    }
...
void SDNDataPlane::handleConfiguration() {
    ...
    // Check if new AP password is provided
    String newAPPassword = configData["newAPPassword"].as<String>();
    if (newAPPassword != "") {
        // Save new AP password for future use
        saveAPPassword(newAPPassword);
    }
...
void SDNDataPlane::saveAPPassword(String password) {
    File file = SPIFFS.open("/ap_password.txt", "w");
    if (file) {
        file.print(password);
        file.close();
    }
...
String SDNDataPlane::loadAPPassword() {
    File file = SPIFFS.open("/ap_password.txt", "r");
    if (file) {
...
void SDNDataPlane::registerWithControlPlane() {
    ...
    registration["mac"] = WiFi.macAddress();
    registration["readInterval"] = config.readInterval;
    registration["firmwareVersion"] = capability.firmwareVersion;
    registration["hardwareVersion"] = capability.hardwareVersion;
    
    // Add capabilities summary
    if (capability.deviceType == "sensor") {
        JsonArray sensors = registration.createNestedArray("sensors");
        for (int i = 0; i < capability.sensorCount; i++) {
            sensors.add(capability.sensors[i].sensorType);
        }
    } else if (capability.deviceType == "actuator") {
        JsonArray actuators = registration.createNestedArray("actuators");
        for (int i = 0; i < capability.actuatorCount; i++) {
            actuators.add(capability.actuators[i].command);
        }
    }
    ...
    if (httpCode == 200) {
        ...
        Serial.println("Control Plane: " + config.controlPlaneIP);
    } else {
...
String SDNDataPlane::generateDeviceId() {
    // For ESP8266, use MAC address to generate unique ID
    uint8_t mac[6];
    WiFi.macAddress(mac);
    char deviceId[20];
    sprintf(deviceId, "ESP8266_%02X%02X%02X%02X%02X%02X", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
...
String SDNDataPlane::collectSensorData() {
    ...
    // Default implementation with dummy data
